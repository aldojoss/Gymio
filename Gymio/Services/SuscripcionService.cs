...
namespace Gymio.Services
{
    public class SuscripcionService : ISuscripcionService
    {
        ...
        public SuscripcionService(IDbContextFactory<GymioDbContext> contextFactory, IFirebaseService firebaseService)
        {
        ...
        public async Task NotificarSuscripcionesPorVencerAsync()
        {
            using var _context = await _contextFactory.CreateDbContextAsync();
            var fechaLimite = DateTime.Today.AddDays(3);

            var suscripcionesPorVencer = await _context.SuscripcionesClientes
                .Include(s => s.Cliente)
                .Where(s => s.Estado == "Activa" && s.FechaVencimiento.Date == fechaLimite.Date)
                .ToListAsync();

            foreach (var suscripcion in suscripcionesPorVencer)
            {
                if (suscripcion.Cliente != null && !string.IsNullOrEmpty(suscripcion.Cliente.FcmToken))
                {
                    try
                    {
                        var titulo = "Suscripción por Vencer";
                        var mensaje = $"Hola {suscripcion.Cliente.Nombre}, tu suscripción vence en 3 días ({suscripcion.FechaVencimiento.ToShortDateString()}).";
                        
                        await _firebaseService.EnviarPushAsync(suscripcion.Cliente.FcmToken, titulo, mensaje);
                        
                        var notificacion = new NotificacionFirestore
                        {
                            UsuarioId = suscripcion.Cliente.Id,
                            Titulo = titulo,
                            Mensaje = mensaje,
                            Tipo = "Alerta",
                    ...
                    {
                        // Ignorar errores individuales para no detener el proceso de otras notificaciones
                    }
                }
            }
        }
